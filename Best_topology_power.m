%% This script will generate the graph that minimizes cost function composed of 
% convergence time and power terms

% each egent can use the power from 0-1
...
P_eig = 10; % Panalty for convergence time
P_power = 1; % Panalty for power comsumption

% Find the optimal spanning tree
[optimalAdjMatrix,Optvalue,eig_list] = findOptimalGraph_1(positions, distanceMatrix, r, Deg_lim, P_eig, P_power);
...
subplot(2,1,2)
hold on
[w2,inciden2] = findOptimalGraph_2(optimalAdjMatrix, P_eig, P_power);
G = graph(optimalAdjMatrix);
G.Edges.Weight = w2;
...
function  [w,inciden]=findOptimalGraph_2(optimalAdjMatrix, P_eig, P_power)
...
cvx_precision best 
    ...
    minimize(100*P_eig*(10-r)+P_power*(sum(w)))
    %maximize(1*r)
    subject to
        L == A * diag(w) * A';
        r * I <= L+b*ones(n,n) ;
...
function [optimalAdjMatrix,Optvalue,eig_list] = findOptimalGraph_1(positions, distanceMatrix, r, Deg_lim, P_eig, P_power);
    ...
    for k = 1:length(subgraphs)
        ...
        objValue = 10*P_eig*(10 - lambda2) + P_power*totalDistance;
